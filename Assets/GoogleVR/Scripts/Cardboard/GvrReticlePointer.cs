...
public class GvrReticlePointer : GvrBasePointer
{
    ...
    public float maxReticleDistance = 70.0f;

    /// Number of segments making the reticle circle.
    public int reticleSegments = 30;
    ...
    /* START - For purpose of the Timed Gaze Input */
    private float GazeStartTime;
    private GameObject GazeAt;
    /* END - For purpose of the Timed Gaze Input */
    ...
    public override void OnPointerEnter(RaycastResult raycastResultResult, bool isInteractive)
    {
        if (InputMovement.CurrentInputMovementType == InputMovementType.RUN)
        {
            ...
            if (ExecuteEvents.CanHandleEvent<ITimedEventHandler>(raycastResultResult.gameObject))
            {
                this.GazeAt = raycastResultResult.gameObject;

                this.GazeStartTime = Time.time;
                this.CreateReticleVertices(1.0f / 30.0f);
            }
        }
    ...
    public override void OnPointerHover(RaycastResult raycastResultResult, bool isInteractive)
    {
        if (InputMovement.CurrentInputMovementType == InputMovementType.RUN)
        {
            ...
            if (this.GazeAt != null && this.GazeStartTime >= 0.0f)
            {
                if (this.GazeAt.tag == "Bridge") this.HandleHover(Time.time);
                else this.HandleHover(-1.0f);
            }
        }
    ...
    private void HandleHover(float restartValue)
    {
        if (ExecuteEvents.CanHandleEvent<ITimedEventHandler>(this.GazeAt) == true)
        {
            ...
            else this.CreateReticleVertices(currentTime / 3.0f);
        }
    }
    ...
    private void CreateReticleVertices(float fillPercent)
    {
        Mesh mesh = new Mesh();
        gameObject.GetComponent<MeshFilter>().mesh = mesh;

        int segments_count = (int)(reticleSegments * fillPercent);
        ...
        for (int si = 0; si <= segments_count; ++si)
        {
            ...
            float angle = (float)si / (float)(reticleSegments) * kTwoPi;
